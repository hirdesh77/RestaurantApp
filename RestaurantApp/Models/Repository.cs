...
namespace RestaurantApp.Models
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        //addded
        public async Task DeleteByIdAsync<TKey>(TKey id, string propertyName)
        {
            IQueryable<T> query = _dbSet;
            var entity = await query.FirstOrDefaultAsync(e => EF.Property<TKey>(e, propertyName).Equals(id));
            if (entity != null)
            {
                _dbSet.Remove(entity);
                await _context.SaveChangesAsync();
